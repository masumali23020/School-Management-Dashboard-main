...
import { getTeacherSalaryStatus, recordSalaryPayment } from "@/Actions/Salaryactions/Salaryactions";
...
type SalaryStructureItem = { id: number; salaryTypeId: number; salaryTypeName: string; isRecurring: boolean; amount: number };
type TeacherItem = {
  id:       string;
  name:     string;
  surname:  string;
...
type SalaryStatusItem = {
  ...
  totalPaid:      number;
  payments: {
    id: number; invoiceNumber: string; amountPaid: number;
    paymentMethod: string; monthLabel: string | null; paidAt: string; collectedBy: string;
  }[];
};

type Invoice = {
  id: number; invoiceNumber: string; teacherName: string; teacherId: string;
  salaryTypeName: string; amountPaid: number; paymentMethod: string;
  monthLabel: string | null; academicYear: string; paidAt: string; collectedBy: string;
};
...
export default function SalaryCashierClient({
  ...
  const [salaryStatus,   setSalaryStatus]   = useState<{
    salaryStatus: SalaryStatusItem[]; totalConfigured: number; totalPaid: number; academicYear: string;
  } | null>(null);
  ...
  const handleSearch = () => {
    setHasSearched(true);
    if (!query.trim()) { setSearchResults(teachers); return; }
    const q = query.toLowerCase();
    setSearchResults(teachers.filter((t) =>
  ...
  const handleSelect = async (teacher: TeacherItem) => {
    ...
    setSelStructureId(""); setSelSalaryTypeId(""); setPayAmount("");
    setLastInvoice(null); setShowInvoice(false);
  ...
  const handleSelectStructure = (structureId: number) => {
    ...
    const item = salaryStatus?.salaryStatus.find((s) => s.structureId === structureId);
    if (item) { setSelSalaryTypeId(item.salaryTypeId); setPayAmount(String(item.amount)); }
    setPayError("");
  };
  ...
  const handleSelectAdHoc = (salaryTypeId: number) => {
    ...
    setSelStructureId(""); setPayAmount("");
    setPayError("");
  };
...
const handlePay = async () => {
  ...
  const handleDownloadPDF = () => {
    ...
    generateSalaryPDF({
      ...
      teacherId:      lastInvoice.teacherId,
      teacherPhone:   selected?.phone,
  ...
  // All active salary types for ad-hoc
  const adHocTypes = salaryTypes.filter((st) =>
    !configuredStatus.some((cs) => cs.salaryTypeId === st.id)
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wider">🔍 Find Teacher</h2>
            <div className="flex gap-2">
              <input type="text" placeholder="Name, ID or subject…" value={query}
          ...
          <div className="space-y-2">
            <p className="text-xs text-gray-400">{searchResults.length} teacher{searchResults.length !== 1 ? "s" : ""}</p>
            {searchResults.length === 0 ? (
              <div className="text-center py-8 text-gray-400 bg-gray-50 rounded-xl text-sm">No teachers found.</div>
            ) : searchResults.map((t) => (
              <button key={t.id} onClick={() => handleSelect(t)}
                className={`w-full text-left flex items-center gap-3 p-3 rounded-xl border transition-colors ${
                  selected?.id === t.id ? "border-emerald-400 bg-emerald-50" : "border-gray-100 bg-white hover:bg-gray-50"
                }`}>
                <div className="relative w-10 h-10 rounded-full overflow-hidden bg-gradient-to-br from-green-400 to-teal-500 flex-shrink-0">
                  {t.img
                    ...
                    : <span className="absolute inset-0 flex items-center justify-center text-white font-bold text-sm">{t.name[0]}</span>
                  }
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-800 truncate">{t.name} {t.surname}</p>
                  <p className="text-xs text-gray-400 truncate">{t.subjects.slice(0,3).join(", ") || "No subjects"}</p>
                  <p className="text-xs text-gray-400">{t.salaryStructures.length} configured</p>
                </div>
                {selected?.id === t.id && <span className="text-emerald-500 font-bold text-lg">✓</span>}
              </button>
            ))}
          </div>
        </div>
              ...
              <div className="bg-emerald-50 rounded-xl p-4 flex items-center gap-3">
                <div className="relative w-12 h-12 rounded-full overflow-hidden bg-gradient-to-br from-green-400 to-teal-500 flex-shrink-0">
                  {selected.img
                    ...
                    : <span className="absolute inset-0 flex items-center justify-center text-white font-bold">{selected.name[0]}</span>
                  }
                </div>
                <div>
                  <p className="font-bold text-gray-800">{selected.name} {selected.surname}</p>
                  <p className="text-xs text-gray-500">{selected.subjects.join(", ") || "—"}</p>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-sky-50 rounded-lg p-2 text-center">
                      ...
                      <p className="font-bold text-sky-700">৳{salaryStatus.totalConfigured.toLocaleString()}</p>
                    </div>
                    <div className="bg-emerald-50 rounded-lg p-2 text-center">
                      <p className="text-xs text-emerald-400">Paid This Year</p>
                      <p className="font-bold text-emerald-700">৳{salaryStatus.totalPaid.toLocaleString()}</p>
                    </div>
                  </div>
                  ...
                  {configuredStatus.length > 0 && (
                    <div className="rounded-xl border border-gray-100 overflow-hidden">
                      <div className="bg-gray-50 px-3 py-2 border-b border-gray-100">
                        <p className="text-xs font-semibold text-gray-500 uppercase">Configured Salaries</p>
                      </div>
                      <table className="w-full text-sm">
                        <tbody>
                          {configuredStatus.map((s, idx) => (
                            <tr key={s.structureId} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50/40"}>
                              <td className="px-3 py-2 font-medium text-gray-700">{s.salaryTypeName}</td>
                              <td className="px-3 py-2 text-gray-500 text-xs">৳{s.amount.toLocaleString()}</td>
                              <td className="px-3 py-2">
                                {s.totalPaid >= s.amount
                                  ? <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">✅ Paid</span>
                                  : s.totalPaid > 0
                                  ? <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">Partial ৳{s.totalPaid}</span>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                    ...
                    {configuredStatus.length > 0 && (
                      <div>
                        <label className="text-xs text-gray-500 font-medium block mb-1">From Configured Salary</label>
                        <select value={selStructureId}
                          onChange={(e) => e.target.value ? handleSelectStructure(Number(e.target.value)) : (setSelStructureId(""), setSelSalaryTypeId(""), setPayAmount(""))}
                          ...
                          {configuredStatus.map((s) => (
                            <option key={s.structureId} value={s.structureId}>
                              {s.salaryTypeName} — ৳{s.amount.toLocaleString()}
                            </option>
                          ))}
                        </select>
                      </div>
                    ...
                    {/* Or ad-hoc type (e.g. Eid Bonus not in structure) */}
                    <div>
                      <label className="text-xs text-gray-500 font-medium block mb-1">Or Ad-hoc / Bonus Type</label>
                      <select value={!selStructureId ? selSalaryTypeId : ""}
                        onChange={(e) => e.target.value ? handleSelectAdHoc(Number(e.target.value)) : (setSelSalaryTypeId(""), setPayAmount(""))}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300 bg-white">
                        <option value="">Select type for ad-hoc payment…</option>
                        {salaryTypes.map((st) => (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        <label className="text-xs text-gray-500 font-medium block mb-1">Method</label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden"
            ...
            <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-5 text-center">
              ...
              <p className="text-emerald-200 text-xs uppercase tracking-widest">Payment Successful</p>
