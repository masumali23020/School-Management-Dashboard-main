...
export const createStaff = async (state: CreateState, data: StaffSchema) => {
  try {
    await createEmployeeInClerkAndDB(data, UserRole.STAFF);
    return { success: true, error: false };
  } catch (err) {
...
export const updateStaff = async (state: CreateState, data: StaffSchema) => {
  if (!data.id) return { success: false, error: true };
  try {
    await updateEmployeeInClerkAndDB(data.id, data);
    return { success: true, error: false };
...
export const deleteStaff = async (state: CreateState, formData: FormData) => {
  const id = formData.get("id") as string;
  try {
    await deleteEmployeeFromClerkAndDB(id);
    return { success: true, error: false };
