...
import {
  ...
  CheckSquare, Square, Search, BookOpen,
} from "lucide-react";
import TableSearch from "@/components/TableSearch";
import Pagination from "@/components/Pagination";
import { createExamsBulk, deleteExamAction, getSubjectsForClass, updateExamAction } from "@/app/actions/examActions/examActions";
...
export default function ExamPageClient({ exams, count, page, role, classes }: Props) {
          ...
          <tbody>
            {exams.length === 0 ? (
              ...
              exams.map((exam) => (
                ...
                >
                  <td className="p-3 font-medium">{exam.lesson.subject.name}</td>
...
function CreateExamModal({
  ...
  const [selectedIds, setSelectedIds] = useState<Set<number>>(
    new Set(classes.map((c) => c.id))   // all selected by default
  );
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            className="border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-blue-400"
          />
        </div>
      ...
      <button
        ...
        disabled={submitting}
        className="w-full flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-white py-2.5 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {submitting
          ? <><Loader2 className="h-4 w-4 animate-spin" /> Creating exams…</>
          : <><BookOpen className="h-4 w-4" /> Create Exam</>
        }
      </button>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-white py-2.5 rounded-lg font-medium transition-colors disabled:opacity-50"
      >
