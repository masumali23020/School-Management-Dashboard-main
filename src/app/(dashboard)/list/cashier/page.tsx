...
type CashierPageType = Employee ;



const CashierPage = async({searchParams}: {searchParams: {[key: string]: string | undefined}}) => {
...
...(role === "admin" ) ? [ {
   ...
   const renderRow = (item: CashierPageType) =>  (
        <tr
      key={item.id}
    ...
    >
      ...
      <td>
        <div className="flex items-center gap-2">
          <Link href={`/list/cashier/${item.id}`}>
...
// url params conditions 
  ...
  const [cashierData, count] = await prisma.$transaction([
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">All Cashier ({count})</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            {role === "admin" && (
              ...
              // </button>
              <FormContainer table="cashier" type="create" />
            )}
          </div>
        </div>
      </div>
      {/* LIST */}
      <Table columns={columns} renderRow={renderRow} data={cashierData} />
      {/* PAGINATION */}
